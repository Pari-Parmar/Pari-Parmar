# Hi there, I'm Parisha Parmar 👋

![Header Banner](https://capsule-render.vercel.app/api?type=waving&color=gradient&customColorList=1,11,21,31&height=180&section=header&text=Parisha%20Parmar&fontSize=38&fontColor=ffffff&fontAlignY=40)

[![Real-time Typing](https://readme-typing-svg.demolab.com?font=Fira+Code&weight=600&size=20&duration=3000&pause=1000&color=38BDF8&center=true&vCenter=true&width=620&lines=Frontend+%26+UI%2FUX+Developer+🎨;B.Tech+CSE+Student+(7th+Sem)+🎓;Crafting+Clean+%26+Intuitive+Experiences+✨;Working+with+Modern+APIs+%26+React+💻)](https://github.com/Pari-Parmar)

**✨ Crafting intuitive, responsive user experiences and bridging them with modern APIs ✨**

![Live Views](https://komarev.com/ghpvc/?username=Pari-Parmar&style=for-the-badge&color=6366f1&label=LIVE+PROFILE+VIEWS)
![Followers](https://img.shields.io/github/followers/Pari-Parmar?label=Followers&style=for-the-badge&color=3b82f6&logo=github)
![Status](https://img.shields.io/badge/Status-Designing%20%26%20Building-6366f1?style=for-the-badge)

---

## 👩‍💻 About Me

Hi! I am **Parisha Parmar**, a **Frontend & UI/UX Developer** and a Computer Science & Engineering student in my **7th semester**.

I specialize in crafting visually stunning, responsive, and intuitive user interfaces (UI/UX) while seamlessly integrating and working with backend APIs. I love participating in hackathons, turning rough wireframes into clean, interactive web applications, and constantly elevating user experiences.

- 🎓 **Education:** Pursuing B.Tech in Computer Science & Engineering (7th Semester).
- 🎨 **What I Do:** Frontend & UI/UX Development with **React, JavaScript/TypeScript, Tailwind CSS**, and **Backend REST API integration**.
- 🏆 **Hackathons:** Active builder in events like **Odoo Hackathon 2026**, **Gateway Hackathon**, and **Gujarat Police Hackathon**.
- 💡 **Philosophy:** Great software starts with thoughtful design, smooth usability, and clean, modular code.

---

## 🛠️ My Technical Skills

### 🎨 UI/UX & Frontend Development
- User Interface (UI) Design, User Experience (UX), Wireframing, Responsive Design
- React.js, JavaScript (ES6+), TypeScript, HTML5, CSS3
- Tailwind CSS, Bootstrap, Mobile-First Design
- Vite, npm

### ⚙️ API Integration & Backend Collaboration
- Working with RESTful APIs, JSON data structures, Postman
- Integrating frontend apps with Node.js, Express, and FastAPI backend services
- Basic database familiarity: MongoDB, MySQL

### 🧰 Tools & Workflow
- Git, GitHub, VS Code, Vercel

---

## 🚀 Featured Projects

| Project | Live Demo | Repository | Description |
| :--- | :---: | :---: | :--- |
| 🌍 **GlobeTrotter** | [🌐 Live Website](https://odoo-hackathon-globe-trotter.vercel.app) | [💻 View Code](https://github.com/Pari-Parmar/Odoo-Hackathon-GlobeTrotter) | Multi-city personalized travel itinerary planning app built for **Odoo Hackathon 2026** with an interactive UI. |
| ⚡ **Gateway Hackathon Project** | [🌐 Live Website](https://gateway-hackathon-final.vercel.app) | [💻 View Code](https://github.com/Pari-Parmar/Gateway-Hackathon_Final) | Full-featured frontend web app built during the Gateway Hackathon solving modern workflow issues. |
| 🎓 **EDTech Platform** | Demo in Repo | [💻 View Code](https://github.com/Pari-Parmar/EDTech) | Comprehensive interactive learning portal featuring course modules, structured lessons, and user dashboard. |
| 🛡️ **Threat Scanner** | CLI / Utility | [💻 View Code](https://github.com/Pari-Parmar/Threat-Scanner) | Security assessment utility designed to identify system vulnerabilities and configuration issues. |
| 🚓 **Police Surveillance Analytics** | In Active Development | [💻 Repository](https://github.com/Pari-Parmar) | Real-time smart surveillance dashboard built with modern React frontend and FastAPI for the **Gujarat Police Hackathon**. |

---

## 📊 Real-Time GitHub Analytics

### Live GitHub Stats
![Parisha's GitHub Stats](https://github-readme-stats.vercel.app/api?username=Pari-Parmar&show_icons=true&theme=tokyonight&include_all_commits=true&count_private=true&hide_border=true)

### Most Used Languages
![Top Languages](https://github-readme-stats.vercel.app/api/top-langs/?username=Pari-Parmar&layout=compact&theme=tokyonight&hide_border=true)

### Live Contribution Streak
![Live Streak Stats](https://streak-stats.demolab.com?user=Pari-Parmar&theme=tokyonight&hide_border=true)

### Real-Time 30-Day Activity Graph
![Real-time Activity Graph](https://github-readme-activity-graph.vercel.app/graph?username=Pari-Parmar&theme=tokyo-night&hide_border=true&area=true)

### Daily Developer Quote
![Dynamic Daily Developer Quote](https://quotes-github-readme.vercel.app/api?type=horizontal&theme=tokyonight)

---

## 📬 Let's Connect

I am always happy to connect with other developers and designers, discuss UI/UX ideas, or collaborate on hackathons:

- **LinkedIn:** [linkedin.com/in/pari-parmar](https://www.linkedin.com/in/pari-parmar)
- **GitHub:** [github.com/Pari-Parmar](https://github.com/Pari-Parmar)
- **Email:** [parmarrparisha@gmail.com](mailto:parmarrparisha@gmail.com)

---

⭐ **Thank you for visiting! Feel free to explore my repositories or drop me a message.**
